...
const CategoryFilterDropdown = ({ categories, selected, onChange }) => {
  ...
  const items = useMemo(
    () => (Array.isArray(categories) ? categories : []).map(c => ({
      ...
      name: c.name || c.categoryName || 'Untitled',
    })),
    [categories]
  );
  ...
  const label = allSelected
    ? 'All Categories'
    : selected.length === 1
      ? (items.find(i => i.id === selected[0])?.name || '1 Category')
      ...
      {open && (
        ...
        >
          ...
          <div className="max-h-[240px] overflow-y-auto">
            {filtered.length === 0 ? (
              <div className="px-3 py-4 text-[12px] text-slate-400 text-center">No categories</div>
            ) : (
              filtered.map(opt => {
